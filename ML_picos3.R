...
datatrainlist <- lapply(as.character(train_set$fullfile), function(x)read.table(x, header = TRUE, sep = ","))
#datatrainfr <- bind_cols(datatrainlist)

Output_train_list <- as.character(train_set$file)
...
# linear regression model for background determination
fitbkground <- lapply(seq(1:length(diff1)), function(x) {
  lapply(seq(1:5), function(y) {
    lm(smooth ~ V1, data = subzona[[x]][[y]])
  })
})

# zonecalc_train has the results of looking for a peak into each zone in each diffractogram
# its structure is [[#diffractogram]][[#zone]][[parameter]] where parameter is
# = 1, Maximum in zone, = 2, 2theta where Max is, = 3, background lvl at Maximum, 
# = 4, sd of background measured at subzone as the sd of the residuals of the lm regression
# = 5, prominence, defined as (max - bknd) / sd
zonecalc_train <- lapply(seq(1:length(diff1)), function(x){
  lapply(seq(1:5), function(y) {
    temp_max <- max(zona[[x]][[y]]$smooth)
    temp_2thetamax <- zona[[x]][[y]]$V1[which.max(zona[[x]][[y]]$smooth)]
    temp_bknd2theta <- as.numeric(fitbkground[[x]][[y]]$coefficients[1]) + as.numeric(fitbkground[[x]][[y]]$coefficients[2]) * temp_2thetamax
    temp_sdbknd <- sd(fitbkground[[x]][[y]]$residual)
    list(temp_max, 
         temp_2thetamax, 
         temp_bknd2theta, 
         temp_sdbknd, 
...
temp_df <- data.frame(matrix(unlist(zonecalc_train), byrow=TRUE),stringsAsFactors=FALSE)

Output_train_list[, c("zona1_Max", "PredPicoM1",
                      "zona2_Max", "PredPicoM2", 
                      "zona3_Max", "PredPicoM3", 
                      "zona4_Max", "PredPicoM4",
                      "zona5_Max", "PredPicoM5")] <- NA

Output_train_list$zona1_Max <- temp_df[seq(1, nrow(temp_df), 25), ]
Output_train_list$PredPicoM1 <- temp_df[seq(5, nrow(temp_df), 25), ]
Output_train_list$zona2_Max <- temp_df[seq(6, nrow(temp_df), 25), ]
Output_train_list$PredPicoM2 <- temp_df[seq(10, nrow(temp_df), 25), ]
Output_train_list$zona3_Max <- temp_df[seq(11, nrow(temp_df), 25), ]
Output_train_list$PredPicoM3 <- temp_df[seq(15, nrow(temp_df), 25), ]
Output_train_list$zona4_Max <- temp_df[seq(16, nrow(temp_df), 25), ]
Output_train_list$PredPicoM4 <- temp_df[seq(20, nrow(temp_df), 25), ]
Output_train_list$zona5_Max <- temp_df[seq(21, nrow(temp_df), 25), ]
Output_train_list$PredPicoM5 <- temp_df[seq(25, nrow(temp_df), 25), ]

rm(temp_df)

#intsd <- lapply(seq(1:5), function(x) {intsd[[x]] <- intsd[[x]][,c(FALSE, TRUE)]})

#Output_train_list <- Output_train_list %>% mutate(sd1 = transpose(data.frame(lapply(intsd[[1]], function (x) sd(x)))), 
...
#  mutate(Maximo1 = sapply(intsd[[1]], function (x) max(x)), 
...
# above: deprecated in favor of list of lists
...
for(i in 1:round(ncol(diff1)/2)) {
  ...
  fitGauss <- as.list(fitG(zona[[1]][[2*i-1]], zona[[1]][[2*i]], validos_filt$twothetaM[1], 1, 1, Output_train_list$media1[[1]][i]))
...
#  lines(zona1[[2*i-1]], as.numeric(peakpar[3]) * dnorm(zona1[[2*i-1]], as.numeric(peakpar[1]), as.numeric(peakpar[2])) + Output_train_list$media1[[1]][i], col = "red", pch = 20)
  ...
  fitGauss <- as.list(fitG(zona[[2]][[2*i-1]], zona[[2]][[2*i]], validos_filt$twothetaM[2], 1, 1, Output_train_list$media2[[1]][i]))
  peakpar <- as.list(fitGauss$par)
...
#  lines(zona2[[2*i-1]], as.numeric(peakpar[3]) * dnorm(zona2[[2*i-1]], as.numeric(peakpar[1]), as.numeric(peakpar[2])) + Output_train_list$media2[[1]][i], col = "red", pch = 20)
  ...
  fitGauss <- as.list(fitG(zona[[3]][[2*i-1]], zona[[3]][[2*i]], validos_filt$twothetaM[3], 1, 1, Output_train_list$media3[[1]][i]))
  peakpar <- as.list(fitGauss$par)
...
#  lines(zona3[[2*i-1]], as.numeric(peakpar[3]) * dnorm(zona3[[2*i-1]], as.numeric(peakpar[1]), as.numeric(peakpar[2])) + Output_train_list$media3[[1]][i], col = "red", pch = 20)
  ...
  fitGauss <- as.list(fitG(zona[[4]][[2*i-1]], zona[[4]][[2*i]], validos_filt$twothetaM[4], 1, 1, Output_train_list$media4[[1]][i]))
  peakpar <- as.list(fitGauss$par)
...
#  lines(zona4[[2*i-1]], as.numeric(peakpar[3]) * dnorm(zona4[[2*i-1]], as.numeric(peakpar[1]), as.numeric(peakpar[2])) + Output_train_list$media4[[1]][i], col = "red", pch = 20)
  ...
  fitGauss <- as.list(fitG(zona[[5]][[2*i-1]], zona[[5]][[2*i]], validos_filt$twothetaM[5], 1, 1, Output_train_list$media5[[1]][i]))
  peakpar <- as.list(fitGauss$par)
...
for(i in 1:round(ncol(diff1)/2)) {
  ...
  fitGauss <- as.list(fitG(zona[[1]][[2*i-1]], zona[[1]][[2*i]], validos_filt$twothetaM[1], 1, 1, Output_test_list$media1[[1]][i]))
  peakpar <- as.list(fitGauss$par)
  ...
  fitGauss <- as.list(fitG(zona[[2]][[2*i-1]], zona[[2]][[2*i]], validos_filt$twothetaM[2], 1, 1, Output_test_list$media2[[1]][i]))
  peakpar <- as.list(fitGauss$par)
  ...
  fitGauss <- as.list(fitG(zona[[3]][[2*i-1]], zona[[3]][[2*i]], validos_filt$twothetaM[3], 1, 1, Output_test_list$media3[[1]][i]))
  peakpar <- as.list(fitGauss$par)
  ...
  fitGauss <- as.list(fitG(zona[[4]][[2*i-1]], zona[[4]][[2*i]], validos_filt$twothetaM[4], 1, 1, Output_test_list$media4[[1]][i]))
  peakpar <- as.list(fitGauss$par)
  ...
  fitGauss <- as.list(fitG(zona[[5]][[2*i-1]], zona[[5]][[2*i]], validos_filt$twothetaM[5], 1, 1, Output_test_list$media5[[1]][i]))
  peakpar <- as.list(fitGauss$par)
...
write.csv(methods_comp_plot, paste(filetrainpath, "Maximum-Guess_performance",".csv", sep=""), row.names=FALSE)
best_ispeak <- 2.65
